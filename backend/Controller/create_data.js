...
await team1.addMembers(m1);
await team1.addMembers(m2);
await team1.addMembers(m3);
await team1.addMembers(m4);
await team1.addMembers(m5);
await team2.addMembers(m6);
await team2.addMembers(m7);
await team2.addMembers(m8);
await team2.addMembers(m9);
await team2.addMembers(m10);
await team3.addMembers(m11);
await team3.addMembers(m12);
await team3.addMembers(m13);
await team3.addMembers(m14);
await team4.addMembers(m15);
await team4.addMembers(m16);
await team4.addMembers(m17);
await team4.addMembers(m18);
await team4.addMembers(m19);
await team5.addMembers(m20);
await team5.addMembers(m21);
await team5.addMembers(m22);
await team5.addMembers(m23);

//Events
const event1 = await Event.create({
  name: 'Awesome Hackathon',
  type: 'hackathon',
  description: 'Join us for an exciting hackathon where developers come together to build innovative projects within a limited time frame. This event is a great opportunity to showcase your coding skills, collaborate with other enthusiasts, and win amazing prizes!',
  ...
  type: 'workshop',
  description: 'Join our workshop on the latest technologies and best practices in software development. Experienced speakers will cover a variety of topics, and participants will have the opportunity to engage in hands-on sessions. Don\'t miss this chance to enhance your skills and network with industry experts!',
...
await event1.addChallanges(ch1);
await event1.addChallanges(ch2);
await event1.addChallanges(ch3);
await event2.addChallanges(ch4);
await event3.addChallanges(ch5);
await event4.addChallanges(ch6);
await event4.addChallanges(ch7);
...
const j3 = await judges.create({
  name: 'Tech Innovator',
  type: 'ideathon',
  description: 'Innovative thinker with a focus on idea validation and feasibility.',
  location: 'Innovation Hub',
  date: new Date('2024-07-01'),
  startTime: '12:00:00',
  endTime: '20:00:00',
});
const j4 = await judges.create({
  name: 'AI Enthusiast',
  type: 'talk',
  description: 'AI researcher and speaker passionate about the future of artificial intelligence.',
  location: 'AI Symposium Hall',
  date: new Date('2024-08-10'),
  startTime: '15:00:00',
  endTime: '17:30:00',
});
const j5 = await judges.create({
  name: 'Workshop Wizard',
  type: 'workshop',
  description: 'Experienced instructor leading hands-on workshops on coding best practices.',
...
await event1.addJudges(j1);
await event1.addJudges(j2);
await event1.addJudges(j3);
await event2.addJudges(j1);
await event2.addJudges(j4);
await event3.addJudges(j5);
await event3.addJudges(j2);
await event4.addJudges(j1);
...
const s2 = await submetion.create({
  note_project: 6,
  date_participate: new Date('2024-08-01'),
  feedback: 'Good effort, but consider improving code readability for future submissions.',
  });
const s3 = await submetion.create({
  note_project: 9,
  date_participate: new Date('2024-08-02'),
  feedback: 'Outstanding work! Impressed with the creativity and technical depth.',
    });
const s4 = await submetion.create({
  note_project: 7,
  date_participate: new Date('2024-08-02'),
  feedback: 'Solid project, but make sure to address the identified security concerns.',
    });
const s5 = await submetion.create({
  note_project: 8,
  date_participate: new Date('2024-08-03'),
  feedback: 'Great use of technology. Looking forward to seeing your future projects.',
    });
